...
#
# T-1406 reference FastAPI demo: pure-ctypes bridge to libinvoicekit_ffi.
#
# We deliberately go via ctypes (not pyo3 + maturin) so the demo
# stays installable with `pip install fastapi` plus the workspace's
# already-built cargo cdylib. The CI workflow builds the cdylib
# with `cargo build -p invoicekit-ffi`, exports
# INVOICEKIT_FFI_LIB, and runs pytest.
...
def _engine() -> _EngineLibrary:
    # Lazy single instance so importing this module without the
    # cdylib on disk does not crash at import-time. The FastAPI
    # app's startup handler triggers the load.
    global _ENGINE
    if _ENGINE is None:
