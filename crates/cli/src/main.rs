...
fn main() -> ExitCode {
    ...
    if matches!(sub.as_str(), "--help" | "-h") {
        print!("{USAGE}");
        return ExitCode::SUCCESS;
    }
    if let Some(code) = invoicekit_cli::commands::dispatch(&sub, &rest) {
        return code;
    }

    eprintln!("invoicekit: unknown subcommand {sub:?}");
    eprintln!();
    eprint!("{USAGE}");
    ExitCode::from(2)
}
