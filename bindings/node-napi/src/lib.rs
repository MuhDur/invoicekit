...
//! `invoicekit-binding-node-napi` — Node delivery wrapper over the engine ABI.
//!
//! The TypeScript SDK bead will add the napi-rs package surface. This crate
//! already runs the shared engine ABI golden fixture so the Node binding track
//! is pinned to the byte contract.

/// Process an Engine ABI JSON request through the Node binding wrapper.
///
/// # Examples
///
/// ```
/// let response = invoicekit_binding_node_napi::process_engine_abi_json(
...
///
/// ```
/// assert_eq!(invoicekit_binding_node_napi::crate_name(), "invoicekit-binding-node-napi");
/// ```
#[must_use]
...
mod tests {
    ...
    fn crate_name_is_cargo_package_name() {
        assert_eq!(crate_name(), "invoicekit-binding-node-napi");
    }
    ...
    #[test]
    fn node_wrapper_matches_engine_abi_golden_fixture() {
