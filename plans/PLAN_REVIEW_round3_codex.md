...
session id: 019e6460-a55f-7992-bda3-a82022420a2b
--------
user
This is review round 3 of an InvoiceKit implementation plan. Rounds 1 and 2 have already integrated:
- Dual native+WASM artifacts (NOT WASM-everywhere)
- Layered invoice model with polymorphic URN-keyed JurisdictionExtension (not hardcoded country struct)
- Money/tax/codelists as first-class crates
- Per-domain JVM validator sidecars (not single monolithic JVM container)
- Validator orchestrator with multiple backend types (rust-native, jvm:*, rest:official, partner, cli, none)
- Cassette-replay sandbox (not 60 hand-written mock gateways)
- Phase 2.5 country feasibility manifests before any country crate
- 3 archetype country crates sequentially (Peppol, async clearance, cryptographic) then parallel agent work
- Realistic time estimates (6-9mo GA, 12-18mo broad beta, 24-36mo 60-country SLA)
- AI intake pushed to Phase 6 (after outbound is GA)
- ZATCA revised from 3 to 6-8 weeks; Typst PDF/A-3 from 1 to 4 weeks
- LocalSignerProxy / invoicekit-signer-agent for on-premise HSM/smart card signing
- "Court-grade" terminology removed
- Feature-flagged WASM (cargo build --features=de,fr only)
- Honest success criteria (5 sandbox-proven + 15 manifested, not 60 GA)
- New risks: sandbox unavailability, local credential requirements, rule drift maintenance
- Agent work unit discipline (manifest + fixtures + capability target + done criteria required before agent starts)

This is round 3. Look for SUBTLE issues now — the structural ones should be solved. Specifically:

1. Are dependencies (T-* edges) correct? Look for hidden cycles, missing deps, orphan tasks.
2. Is anything still under-specified for an agent to pick up cold? Pick 3 random tasks and check.
3. Are there hidden coupling problems between archetypes that we haven't addressed?
4. Does the feature-flag system for WebAssembly actually work given the rule-pack-needs-validator-backend coupling?
5. Is there a way to weaponize the cassette-replay sandbox concept further — for example, contract tests for gateway adapters?
6. Anything else that bothers you that wouldn't be caught by the obvious checks?

If round 2 already caught the structural issues, round 3 should produce polish-level changes — typo fixes, dependency-edge corrections, clarifications. If you see another structural issue, surface it loudly.

Be opinionated. Use git-diff style. Don't filler.

Plan follows:

# InvoiceKit — Implementation Plan v0.4 (Global, round-2 integrated)

> Consolidated plan with global country coverage and two rounds of multi-model review (Codex × Gemini) integrated. Replaces v0.1 through v0.3. Round-2 changes: polymorphic invoice-extension model, feature-flagged WebAssembly builds, per-domain JVM validator sidecars, cassette-replay sandbox, country feasibility manifests before any country crate, archetype-first then parallel agent work, realistic time estimates, on-premise signer agent, "court-grade" claims removed.
...
**Feature-flagged WebAssembly builds.** Bundling all 60 country crates into a single WebAssembly artifact would produce a 30+ megabyte payload — too large for Cloudflare Workers (1 megabyte after compression) and slow to start in browsers. WebAssembly builds are therefore feature-flagged: a developer compiles only the countries they need with `cargo build --features="de,fr,it,peppol" --target=wasm32-unknown-unknown`. The core engine (canonicalization, EN 16931 validator, layered invoice model) is always included; per-country format families and per-country report crates are opt-in.
...
A country crate cannot start without its manifest. Agents can produce manifests in parallel; manifests are cheap (1–3 days each).

**Phase 3 — National clearance and reporting crates:**

Three archetype crates first, **sequentially**, to lock in the trait boundaries and the test-cassette workflow:

1. **`country-pl-ksef`** — async clearance archetype (submit, poll, reserve invoice number, commit, cancel, correct). 3 weeks.
2. **`country-sa-zatca`** — cryptographic archetype (ECDSA secp256k1 signing, UBL canonicalization, TLV QR code generation, certificate management). 6–8 weeks. (Earlier estimate of 3 weeks was wrong; ZATCA Phase 2 is genuinely heavy.)
3. **`country-be-pep`** — Peppol-mandate archetype (mostly reuses Family A with country-specific CIUS overlay). 1 week.

Only after the three archetypes are in place — including cassette tests, archetype traits, and documentation — are remaining country crates unleashed to agents in parallel. Each subsequent crate is structured by the archetype it most resembles. Realistic order based on regulatory urgency, market size, and onboarding difficulty:

- **Wave 1** (regulatory urgency, weeks 17–22): Italy SDI, France PA-PDP, Spain VeriFactu and FacturaE, Greece myDATA, UAE PINT-AE national onboarding.
- **Wave 2** (large markets, weeks 22–28): India IRP and e-Waybill, Mexico CFDI, Brazil NF-e and NFS-e, Malaysia MyInvois, Turkey e-Fatura, Romania, Hungary, Japan.
- **Wave 3** (rest of Latin America, MENA, APAC, Africa, weeks 28–36): Chile, Colombia, Peru, Argentina, Ecuador, Costa Rica, Dominican Republic; Egypt, Israel, Jordan, Bahrain; Indonesia, Philippines, Vietnam, Thailand, South Korea, Japan (Qualified Invoice System), Taiwan, China; Kenya, Nigeria, South Africa; Portugal (national reporting alongside Peppol).
...
| T-053 | PDF/A-3 dictionary post-processing (Typst does not natively write all XMP metadata or ZUGFeRD-grade attachments; upstream pull requests or `lopdf` post-processing to fix XMP, attachment relationship, and metadata) | 4 weeks | T-052 |
...
| T-083a | `invoicekit-signer-agent` — open-source on-premise signing proxy for enterprise customers whose qualified certificates, hardware security modules, or smart cards cannot legally leave their datacenter. Engine calls signer over a local Unix socket or HTTPS endpoint with a stable signing API. Required for Poland KSeF, Saudi Arabia ZATCA, Mexico CFDI, Italy SDI for many enterprise customers. | 2 weeks | T-083 |
...
| `country-sa-zatca` | Saudi Arabia ZATCA Phase 2 (ECDSA secp256k1 signing, custom UBL canonicalization, TLV QR generation, certificate management — built as cryptographic archetype) | 6–8 weeks |
...
codex
I’ll treat this as a plan review, not an implementation task. I’m going to sanity-check dependency shape and agent handoff criteria against the supplied v0.4 text, then give only proposed deltas and blockers.
hook: PreToolUse
hook: PreToolUse
hook: PreToolUse Completed
hook: PreToolUse Completed
exec
/bin/zsh -lc 'cm context "InvoiceKit implementation plan v0.4 round 3 dependency and agent-readiness review" --json' in /home/durakovic/projects/invoices
hook: PostToolUse
 ...
 succeeded in 303ms:
{
  "success": true,
  "command": "context",
  "timestamp": "2026-05-26T13:02:11.840Z",
  "data": {
    "task": "InvoiceKit implementation plan v0.4 round 3 dependency and agent-readiness review",
    "relevantBullets": [
      {
        "id": "b-mmdcfmq4-vi0ic7",
        "scope": "global",
        "category": "multi-agent",
        "content": "Wave-based multi-agent execution (Wave 1: fix bugs, Wave 2: rewire API, Wave 3: wire pipeline) is better than dispatching all agents at once. Later waves depend on type/API changes from earlier waves. Respect dependency order.",
        "source": "learned",
        "type": "rule",
        "isNegative": false,
        "kind": "workflow_rule",
        ...
        "createdAt": "2026-03-05T10:49:26.476Z",
        "updatedAt": "2026-03-05T10:49:26.476Z",
        "pinned": false,
        "deprecated": false,
        "sourceSessions": [
          "manual-cli"
    ...
    "suggestedCassQueries": [
      "cass search 'invoicekit implementation plan' --days 30",
      "cass search 'invoicekit error' --days 60",
      "cass search 'invoicekit' --days 90",
      "cass search 'invoicekit implementation' --days 60",
      "cass search 'plan' --days 90"
    ]
  },
  "metadata": {
    "executionMs": 111,
...
One structural issue remains: Track 8 still contradicts the archetype-first rule. The prose says three archetypes are sequential before parallel agents; the task table still starts Wave 1 in parallel at weeks 14-18 and lists archetypes beside ordinary country crates. That will cause agents to start on unstable trait boundaries.

```diff
@@ Section 6 / Track 8
- Each country crate is roughly one to three weeks of work depending on complexity. Agents work in parallel — one crate per agent at a time.
+ Country crates do not start from the generic foundation alone. They require:
+ 1. that country's Phase 2.5 feasibility manifest,
+ 2. cassette recorder/scrubber/matcher framework,
+ 3. the relevant archetype trait locked by one completed archetype crate.

- Wave 1 — Regulatory urgency (weeks 14–18 if parallel):
+ Archetype lock-in (sequential, weeks 14–25/27):
+ `report-pl-ksef` -> async clearance archetype
+ `report-sa-zatca` -> cryptographic signing/stamping archetype
+ `report-be-pep` -> Peppol mandate/CIUS overlay archetype
+
+ Wave 1 — Regulatory urgency (parallel only after archetype lock-in):
```
...
**Dependency Edge Fixes**

```diff
@@ Track 2
- T-030 | Validator worker container (KoSIT, phive, Saxon, JSON-RPC API) | 2 weeks | T-001
+ T-030 | Validator sidecar protocol + per-domain workers (`validator-kosit`, `validator-phive`, `validator-saxon`) | 2 weeks | T-001, T-032

- T-032 | Validation result schema ... | 1 week | T-031
+ T-032 | Validation result schema ... | 1 week | T-010, T-017

- T-031 | EN 16931 hand-written Rust validator ... | 3 weeks | T-010, T-017
+ T-031 | EN 16931 hand-written Rust validator ... | 3 weeks | T-010, T-017, T-032
```

```diff
@@ Track 1
- T-021 | Property-based round-trip tests (IR ↔ JSON ↔ XML) | 1 week | T-019, T-020
+ T-021 | Property-based canonical JSON tests and placeholder XML canonicalization tests | 1 week | T-019, T-020
+ T-021a | Real IR ↔ UBL/CII/XML round-trip tests | 1 week | T-040, T-041, T-019, T-020
...
+ T-070a | Extensible transmission state model and transition contract | 1 week | T-070

- T-071 | Outbox SQL schema, idempotency model, retry policy, dead-letter states | 2 weeks | T-022, T-070
+ T-071 | Outbox SQL schema, idempotency model, retry policy, dead-letter states | 2 weeks | T-022, T-070a

- T-072 | Transmission worker ... | 2 weeks | T-071
+ T-072 | Transmission worker ... | 2 weeks | T-071, T-073, T-074
```

```diff
@@ Signing
- T-083a | invoicekit-signer-agent ... | 2 weeks | T-083
+ T-083a | Stable signing API + `invoicekit-signer-agent` local proxy | 2 weeks | T-080
+ T-083b | eIDAS qualified signature provider adapter | 2 weeks | T-083a
```

Current dependency wrongly makes ZATCA/KSeF/Mexico/Italy signing wait for eIDAS. The local signer proxy is the substrate; eIDAS is one adapter.

**Feature-Flag / WASM Problem**

The feature-flag story is not complete because rule packs can require `jvm:*`, `cli:*`, `partner`, or `rest:official` validators. A WASM build can include a country serializer but still be unable to validate it locally.

```diff
@@ T-033
- Browser/edge validator capability matrix (which profiles are pure-WASM vs require the worker)
+ Browser/edge validator capability matrix: per country/profile/date, report
+ `serialize`, `local_validate`, `reference_validate`, `requires_service`,
+ `requires_cli`, and `unavailable_in_wasm`.

@@ T-108
- Browser bundle (`wasm-bindgen`...)
+ Browser bundle with feature-gated country crates and runtime backend resolver.
+ External validator backends must return `RequiresExternalBackend`, never panic
+ or silently downgrade to partial validation.
```

Also change the claim:

```diff
- The core engine (canonicalization, EN 16931 validator, layered invoice model) is always included
+ The core engine and pure-Rust EN 16931 subset are always included. Peppol/XRechnung/country reference validation may require server-assisted workers depending on the rule pack backend.
```

**Cassette Sandbox Upgrade**

Make cassettes a contract-test system, not only a mock.

```diff
+ T-074a | Cassette recorder, scrubber, matcher, and scenario metadata schema | 2 weeks | T-070, T-120
+ T-074b | GatewayAdapter contract test suite backed by cassettes | 1 week | T-074a, T-073
+ T-074c | Nightly sandbox drift canary: replay live official/partner sandbox calls, diff normalized responses, open source-watch issues | 1 week | T-006, T-074a
```

Required contract cases: idempotent replay, duplicate submission, timeout, malformed receipt, auth failure, certificate rejection, rate limit, delayed async receipt, unknown response field, gateway maintenance page, and partner error translation.

**Three Cold-Agent Task Checks**

`T-006a` is under-specified. It needs a capability schema, stale/downgrade semantics, route/date dimensions, source confidence rules, and output format for `invoicekit capabilities`.

`T-053` is under-specified. It needs exact acceptance fixtures, veraPDF command, required PDF/A-3 profiles, allowed post-processing library, and a decision rule for “Typst PR vs local `lopdf` patch.”

`report-br-nfe` is under-specified. Brazil is not one clean gateway: NF-e, NFS-e, municipal variance, certificate rules, SEFAZ behavior, partner/native choice, and validator oracle must be pinned in the manifest before any agent touches code.

**Other Nits**

```diff
- services/validator-worker-jvm
...
```diff
- invoicekit verify ... Court-grade re-verification
+ invoicekit verify ... Signed evidence-bundle verification
```

```diff
- T-123 | Differential test harness | 2 weeks | T-035
+ T-123 | Differential test harness | 2 weeks | T-030, T-032, T-031, T-040, T-041
```

`T-035` should consume harness output; the public web UI should not be a prerequisite for the harness.

The plan is close, but I would not let agents start Track 8 until the archetype dependency graph and cassette contract tasks are explicit.
tokens used
55,679
**Round 3 Verdict**
